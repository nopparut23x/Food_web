<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="../assets/bootstrap5/css/bootstrap.min.css">
    <link rel="stylesheet" href="../assets/fontawesome/css/all.min.css">
    <script src="../assets/js/jquery-3.6.0.js"></script>
    <script src="../assets/bootstrap5/js/bootstrap.min.js"></script>
    <script src="../assets/fontawesome/js/all.min.js"></script>
    <title>edit</title>
</head>

<body>

    <div class="container-fluid">
        <div class="row flex-nowrap">
            <div class="col-auto col-md-3 col-xl-2 px-sm-2 px-0 bg-dark">
                <?php include 'slidebar.php'; ?>
            </div>
            <div class="col py-3 d-flex justify-content-center">
                <div class="card my-5 shadow-lg border-0 w-75">
                    <div class="card-body">
                        <form action="" class="mt-3">
                            <center>
                                <img class="rounded-circle" src="data:image/png;base64," width="100px">
                                <input type="file" class="form-control w-25 mt-2" name="profile">
                            </center>
                            <div class="row my-5">
                                <div class="col-lg-6">
                                    <input type="text" name="Firstname" class="form-control" required>
                                    <p>ชื่อจริง</p>
                                </div>
                                <div class="col-lg-6">
                                    <input type="text" name="Lastname" class="form-control" required>
                                    <p>นามสกุล</p>
                                </div>
                                <div class="col-lg-12">
                                    <span>เพศ:</span>
                                    <div class="form-check-inline">
                                        <input type="radio" name="gender" class="form-check-input" id="radio1">
                                        <label class="form-check-label" for="radio1"> หญิง </label>

                                        <div class="form-check-inline">
                                            <input type="radio" name="gender" class="form-check-input" id="radio2">
                                            <label class="form-check-label" for="radio2"> ชาย </label>
                                        </div>

                                        <div class="form-check-inline">
                                            <input type="radio" name="gender" class="form-check-input" id="radio3" checked>
                                            <label class="form-check-label" for="radio3"> อื่นๆ </label>
                                        </div>
                                    </div>
                                </div>

                                <div class="col-lg-6 my-4">
                                    <input type="email" name="email" class="form-control" required>
                                    <p>อีเมล</p>
                                </div>
                                <div class="col-lg-6 my-4">
                                    <input type="password" name="password" class="form-control" required>
                                    <p>รหัสผ่าน</p>
                                </div>

                                <div class="col-lg-12 mt-3">
                                    <input type="text" name="address" class="form-control" required>
                                    <p>ที่อยู่</p>
                                </div>
                                <div class="col-lg-12 mt-3">
                                    <input type="text" name="Telephone" class="form-control" required>
                                    <p>เบอร์โทรศัพน์</p>
                                </div>
                            </div>
                            <center>
                                <button class="btn btn-warning my-3">
                                    เเก้ไขโปรไฟล์
                                </button>
                            </center>

                        </form>
                    </div>
                </div>
            </div>
        </div>
    </div>



</body>

</html>